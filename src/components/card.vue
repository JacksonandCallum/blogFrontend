<template>
  <div class="card-container" v-if="!$common.isEmpty(activityList)">
    <div v-for="(item, index) in activityList" :key="item.id" class="card-item shadow-box wow"
      ...
      <div class="card-body">
        ...
        <div style="display: flex;justify-content: end;align-items: end;margin-bottom: 1px;">
          ...
          <div v-if="!item.isEnd && !item.isSign">
          ...
          <!-- <proButton :info="item.isEnd ? '已结束' : '报名'" :before="'rgb(131, 123, 199)'"
            :after="'linear-gradient(45deg, #f43f3b, #ec008c)'" :class="{ 'disabled-button': item.isEnd }"
            @click="!item.isEnd && handleClick">
          </proButton> -->
        </div>
